    ...
    if ($('#seller-profile-view').length > 0 || $('#seller-profile-link').length > 0) {
        ...
        async function renderSellerProducts() {
            ...
            if (!authToken || !currentUser || currentUser.role !== 'vendedor') {
                $productGrid.html('<p class="text-center text-red-500 col-span-full">You must be logged in as a seller to view your products.</p>');
                return;
            }
            ...
            try {
                ...
                if (!products || products.length === 0) {
                    $productGrid.html('<p class="text-center text-gray-500 col-span-full">You have no products. Add one to get started!</p>');
                    return;
                }
                ...
                $productGrid.html('<p class="text-center text-red-500 col-span-full">An error occurred while loading products.</p>');
            }
        }
        ...
        $productForm.on('submit', async function (event) {
            ...
            // FormData is used to correctly handle file uploads (images).
            ...
            formData.append('type', 'product'); // Hardcoded values as per original code
